const deployFunction = async ({ getNamedAccounts, deployments, ethers }) => {
    ...
    const deployLBPManagerFunctionSignature = await lbpManagerFactoryInstance.interface.getSighash(
        "deployLBPManager"
      );

    await deploy("SignerV2",{
        from: root,
        args: [safeInstance.address, [lbpManagerFactoryInstance.address], [deployLBPManagerFunctionSignature]],
        log: true
    });
}
