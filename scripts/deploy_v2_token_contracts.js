...
const main = async () => {
  ...
  const PrimeTokenV2_Factory = await hre.ethers.getContractFactory(
    "PrimeTokenV2"
  );
  const primeTokenV2 = await PrimeTokenV2_Factory.deploy(
    process.env.GENESIS_MULTISIG,
    utils.parseEther(process.env.V2_INITIAL_SUPPLY)
  );
