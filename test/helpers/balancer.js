...
const getLbpFactoryInstance = async (setup) => {
    const lbPFactoryFactory = await ethers.getContractFactory("LiquidityBootstrappingPoolFactory");
    const lbpFactoryInstance = await lbPFactoryFactory.deploy(setup.vault.address);
...
const getVaultInstance = async (setup) => {
    ...
    const authorizerInstance = await authorizerFactory.deploy(setup.roles.root.address);
...
const getLbpFactory = (setup) => new ethers.ContractFactory(LBPArtifact.abi, LBPArtifact.bytecode, setup.roles.root);
