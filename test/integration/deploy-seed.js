...
describe(">> Deploy new seed with gnosis safe", async () => {
  ...
  context("$ prequesities", async () => {
    ...
    it("transfer seed factory ownership to safe", async () => {
      ...
      expect(
        await setup.seedFactory.connect(setup.roles.prime).owner()
    ...
    it("setup gnosis proxy", async () => {
      ...
      expect(await setup.proxySafe.isOwner(setup.roles.prime.address)).to.equal(
        false
      );
  ...
  context(
    "$ Signer cannot create and execute transaction to deploy new seed using safe contract",
    async () => {
  ...
  context(
    "$ create and execute transaction other than to deploy new seed using safe",
    async () => {
      it("reverts", async () => {
        // here we create a transaction object
        nonce++;
  ...
  context(
    "$ create and execute transaction to deploy new seed using safe from other seedFactory",
    async () => {
      it("reverts", async () => {
        // here we create a transaction object
        nonce++;
