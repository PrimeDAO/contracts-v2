...
const deploy = async () => {
    ...
    setup.lbpFactory = await balancer.getLbpFactoryInstance(
        setup.vault
      );
...
describe("Integration: Deploy LBP using Gnosis Safe", async () => {
    let setup;
    let tokenAddresses, admin, owner, sortedTokens, newOwner, projectAdmin, primeBeneficiary;
    ...
    context("Deploying without signer", async () => {
        context(">> setup gnosis safe", async () => {
            before("!! setup", async () => {
                ...
                ({ root: owner, prime: admin,  beneficiary: newOwner, buyer1: projectAdmin, buyer2: primeBeneficiary} = setup.roles);
        ...
        context("$ deploy LBP", async () => {
            ...
            it("$ deploys LBP using Gnosis Safe", async () => {
                const {data, to} = await setup.wrapperFactory.populateTransaction.deployLBPManager(
                    ...params
                );
                let gasEstimated = await setup.wrapperFactory.estimateGas.deployLBPManager(
                    ...params
                );
    ...
    context("Deploying using Signer", async () => {
        ...
        context(">> Setup Signer", async () => {
            it("$ adds correct contract and function signatures", async () => {
                ...
                const functionSignature = await setup.wrapperFactory.interface.getSighash('deployLBPManager');
        ...
        context(">> Generate Valid Signature from Signer", async () => {
            ...
            it("success", async () => {
                ...
                let gasEstimated = await setup.wrapperFactory.estimateGas.deployLBPManager(...params);
                const trx = [
                    to,
                    zero,
                    data,
