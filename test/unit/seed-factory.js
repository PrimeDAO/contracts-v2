...
describe("SeedFactory", () => {
    ...
    context("» creator is owner", () => {
        before("!! deploy setup", async () => {
            setup = await deploy();
            Seed = await ethers.getContractFactory(
                "Seed",
                setup.roles.root
            );
        ...
        context("» parameters are valid", () => {
            ...
            it("it creates new seed contract", async () => {
                ...
                await expect(seedFactory.deploySeed(
                    dao.address,
                    admin.address,
                    [seedToken.address, fundingToken.address],
                    [softCap, hardCap],
        ...
        context("» changeOwner", () => {
            it("only Owner can change owner", async () => {
                await expectRevert(
                    seedFactory.connect(admin).transferOwnership(setup.roles.buyer1.address),
                    "Ownable: caller is not the owner"
                );
            });
            it("changes owner", async () => {
                await seedFactory.connect(dao).transferOwnership(setup.roles.buyer2.address);
