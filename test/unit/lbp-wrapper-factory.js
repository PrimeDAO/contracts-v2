...
describe(">> Contract: WrapperFactory", () => {
	...
	const ADMIN_BALANCE = [32.667e18, 1500.5714e17].map((balance) => balance.toString());
	const AMOUNTS = [16.667e18, 7503.5714e17].map((amount) => amount.toString());
	...
	context("» deploy LBP WrapperFactory", () => {
		before("!! setup", async () => {
			...
			({ root: owner, prime: admin } = setup.roles);
