...
const getParameters = async (lbpFactoryAddress, tokenAddresses, beneficiary) => {
...
const deploy = async () => {
  ...
  describe("Contract: Signer", async () => {
    ...
    before("!! setup", async () => {
      ...
      uselessFunctionSignature =
          await setup.lbpManagerFactoryInstance.interface.getSighash("setMasterCopy");
      deployLBPManagerFunctionSignature =
          await setup.lbpManagerFactoryInstance.interface.getSighash("deployLBPManager");
    });
      ...
      context("valid constructor parameters", async () => {
        it("deploys signer contract", async () => {
          setup.signer = await Signer_Factory.deploy(
            ...
            [setup.lbpManagerFactoryInstance.address],[uselessFunctionSignature]
          );
    ...
    context(">> approveNewTransaction", async () => {
        beforeEach("!! new Signer v2 instance", async () => {
            setup.newSigner = await Signer_Factory.deploy(
                owner.address,
                [],[]
            );
        });
        it("$ reverts when caller is not safe", async () => {
            await expect(
                setup.newSigner.connect(admin).approveNewTransaction(setup.lbpManagerFactoryInstance.address, uselessFunctionSignature)
            ).to.be.revertedWith("Signer: only safe functionality");
        });
        it("$ reverts when contract address is zero address", async () => {
            await expect(
                setup.newSigner.connect(owner).approveNewTransaction(ZERO_ADDRESS, uselessFunctionSignature)
            ).to.be.revertedWith("Signer: contract address cannot be zero");
        });
        it("$ reverts when function signature is zero", async () => {
            await expect(
                setup.newSigner.connect(owner).approveNewTransaction(setup.lbpManagerFactoryInstance.address, '0x00000000')
            ).to.be.revertedWith("Signer: function signature cannot be zero");
        });
        it("$ approves new transaction when invoked by safe", async () => {
            await expect(
                setup.newSigner.connect(owner).approveNewTransaction(setup.lbpManagerFactoryInstance.address, uselessFunctionSignature)
            ).to.not.be.reverted;
            expect(
                await setup.newSigner.connect(owner).allowedTransactions(setup.lbpManagerFactoryInstance.address, uselessFunctionSignature)
            ).to.equal(true);
        });
    });
    context(">> setSafe", async () => {
        ...
        it("$ sets new safe safe address with valid address", async () => {
            ...
            expect(
                await setup.newSigner.connect(owner).safe()
            ).to.equal(setup.proxySafe.address);
        });
    });
    context(">> generateSignature", async () => {
        before("!! setup with necessary function approved", async () => {
            setup.signer = await await Signer_Factory.deploy(
                setup.proxySafe.address,
                [setup.lbpManagerFactoryInstance.address],[deployLBPManagerFunctionSignature]
            );
            expect(
                await setup.signer.connect(owner).allowedTransactions(setup.lbpManagerFactoryInstance.address, deployLBPManagerFunctionSignature)
            ).to.equal(true);
        });
        ...
        it("reverts on invalid function call which is not approved", async () => {
          ...
          const { data, to } =
            await setup.lbpManagerFactoryInstance.populateTransaction.deployLBPManager(
              ...(await getParameters(setup.lbpFactory.address, tokenAddresses, primeBeneficiary.address))
            );
    ...
    context(">> check if signature is accepted by gnosis safe", async () => {
      ...
      it("safe should accept signer's signature", async () => {
        const { data, to } =
            await setup.lbpManagerFactoryInstance.connect(owner).populateTransaction.deployLBPManager(
              ...(await getParameters(setup.lbpFactory.address, tokenAddresses, primeBeneficiary.address))
            );
        let gasEstimated = await setup.lbpManagerFactoryInstance.connect(owner).estimateGas.deployLBPManager(
            ...(await getParameters(setup.lbpFactory.address, tokenAddresses, primeBeneficiary.address))
          );
